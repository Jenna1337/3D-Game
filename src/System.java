...
public class System
{
	...
	{
		return Math3.evalExt3(quest.trim(), false).toString().replace("`","-");
	}
...
final class Math3
{
	static Object evalExt3(String q, boolean debug) 
	{
		if(debug) System.out.println(q);
		try
		{
			return new SuperInteger(q);
		}
		catch(Exception e)
		{
			try
			{
				...
				query=toAdd+query;
				String n1,n2;
				SuperInteger d1=new SuperInteger(0), d2=new SuperInteger(0);
				String[] vals;
				char op;
				Character[][][] opsWithVars = 
					{
						{
							{'^',2}
							//TODO add roots
				...
				Character[][] ops = getOps(opsWithVars);
				int index;
				...
				for(int i=0;i<ops.length;++i)
				{
					while(containsAny(query,ops[i]))
					{
						...
						if((vars==2) || ((vars==1)&&(props[2].equals('L'))))
							d1 = new SuperInteger(n1.replace("`","-"));
						if((vars==2) || ((vars==1)&&(props[2].equals('R'))))
							d2 = new SuperInteger(n2.replace("`","-"));
	...
	private static Character[] getFirst(String query, Character[][] groups, boolean debug)
	{
		...
		for(int i=0; i<loc.length; ++i)
		{
			...
			if(loc[i][0]>loc[i][1])
			{
				System.err.println(new ArithmeticException("Mismatched dilimiters"));
				System.exit(1);
			}
		}
		if(debug)
	...
	private static String specialOp(String query, char var)
	{
		...
		{
			case 'a':
				return SuperInteger.abs(new SuperInteger(query)).toString();
			default:
				System.err.println(new UnsupportedOperationException(var+""));
				System.exit(1);
				break;
		}
		return query;
	...
	private static Character[] getProps(Character[][] varList, char op)
	{
		for(int i=0; i<varList.length; ++i)
			if(varList[i][0].equals(op))
				return varList[i];
		new Exception().printStackTrace();
		return new Character[] {'\u0000',0};
	}
	private static Character[][] getOps(Character[][][] opsWithVars)
	{
	...
	private static int getOpsIndex(String query, Character[] ops)
	{
		for(int i=0;i<query.length();++i)
			if(equalsAnyOp(query.charAt(i)+"",ops))
				return i;
		System.err.println(new Exception("No valid operator was found. "));
		System.exit(1);
		return -1;
	}
	private static String SingleOp(SuperInteger n1, char op, SuperInteger n2)
	{
		SuperInteger out = new SuperInteger(n1.toString());
		switch(op)
		{
			case '*':
				out.multiply(n2);
			...
			case '^':
				...
				break;
			default: 
				System.err.println(new UnsupportedOperationException(op+""));
				System.exit(1);
				return "";
		}
	...
	{
		for(int i=0;i<b.length;++i)
			if(a.toString().contains(b[i].toString()))
				return true;
		return false;
	}
	private static boolean containsAny2(Object a, Object[][] b, boolean debug)
	{
		for(int i=0;i<b.length;++i)
			for(int j=0;j<b[i].length;++j)
				if(a.toString().contains(b[i][j].toString()))
					return true;
		return false;
	}
